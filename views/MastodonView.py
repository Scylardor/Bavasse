...
class MastodonView(object):
    ...
    def Create(self, controller, row_idx):
        self.titleButton = tk.Button(self.parent, text="Mastodon", command=controller.ToggleNewClientMenu).grid(row=1 + row_idx, column=2, pady=10)
        ...
        def AddNewClientCallback():
            controller.AddNewClient(self.masto_client_id_entry.get())
        ...
        def ConnectClientOAuthCallback():
            controller.ConnectClientOAuth(oauth_token=self.oauth_token_entry.get())
